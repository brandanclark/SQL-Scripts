...
WHERE ((CONVERT(float, (REPLACE([ACoS],'%', ''))) < '25'
	...
	OR [Placement Type] = 'Top of search (page 1)'
...
UPDATE [TwinScrollsMarketing].[dbo].[SponsoredProductsCampaigns_Updates] 
	SET [Increase bids by placement] = '50%'
		,[Notes] = 'The Top of search (page 1) Campaign Placement "' + CONVERT(VARCHAR(50), [Campaign]) + '" increase has been set to 50%'
	,[Brand] = CASE 
		WHEN [Campaign] LIKE '%-%'
			THEN LEFT(Campaign, CHARINDEX('-', Campaign + '-') - 1)
		ELSE Campaign
	...
	,[Product Group] = CASE 
		...
		ELSE Campaign
		END
	WHERE [Placement Type] = 'Top of search (page 1)'
		AND CONVERT(FLOAT, (REPLACE([Increase bids by placement], '%', ''))) > '50'
