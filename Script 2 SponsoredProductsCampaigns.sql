...
/**
Script for Step 2 of changes to Sponsored Products Campaigns table. Decrese Max Bid for products with high ACoS numbers and high clicks numbers
Steps:
...
SELECT * FROM [TwinScrollsMarketing].[dbo].[SponsoredProductsCampaigns_ImportTableBackup] WHERE  CONVERT(float, (REPLACE([ACoS],'%', ''))) > @ACoSTrigger
...
SET [Max Bid] = (CASE WHEN [Max Bid] = '' THEN CONVERT(decimal(18,2),(SELECT MAX(CONVERT(float, [Max Bid])) 
					...
					END)  
	,[Notes] = 'The Sponsored Product ' + [Match Type] + ' match keyword' + ' "' +
